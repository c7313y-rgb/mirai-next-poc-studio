...
const nav=[{to:'/',label:'ダッシュボード',icon:'▦'},{to:'/curriculum',label:'教材ライブラリ・編集',icon:'▤'},{to:'/lessons',label:'授業の進行',icon:'▷'},{to:'/classes',label:'クラス・生徒の記録',icon:'◎'},{to:'/themes',label:'探究テーマの配信',icon:'✦'},{to:'/lesson-surveys',label:'授業後アンケート',icon:'☑'}];
export default function TeacherApp(props){const p=props.route.path;let page,m;
...
return <Shell {...props} nav={nav}>{page}</Shell>;
}
    ...
    <div className="stack">
      <div className="spread">
        ...
        {data?.pendingLessonSurveys > 0 && (
          <Link to="/lesson-surveys" className="notice warn" style={{ textDecoration: 'none', fontWeight: 700 }}>
      ...
      {data?.classes.length === 0 && <div className="panel">担当クラスが登録されていません。運営（Edutex）にご連絡ください。</div>}
      <div className="cards">
        {data?.classes.map((c) => (
          <Link key={c.id} to={`/classes/${c.id}`} className="card" style={{ textDecoration: 'none', color: 'inherit' }}>
            <div className="label">{c.school_name}</div>
            <div className="value">{c.grade}年{c.name}組</div>
